...
トランザクション内の一部のSQL文だけコミットできる

 不正解 「トランザクションを途中で取り消すにはROLLBACK文を使用する 」が正解
 
トランザクションを途中で取り消すにはROLLBACK文を使用する
...
このテーブルを削除するには「DROP employee」を実行する

 不正解 「行挿入時にname列の値を指定しなかった場合、その行のname列の値はnullになる 」が正解
...
連関エンティティを導入し、1対多のリレーションシップに解決する
...
新システムを構築する場合、トップダウン・アプローチを主軸にボトムアップ・アプローチを組み合わせるのが一般的である
